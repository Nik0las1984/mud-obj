...
switch %object.vnum%
  case 26001
    ...
    case 26003
      ...
      say Я продолжу свой рассказ, но буду говорить немного тише.
...
if !(%nut260% ==2) && !(%nut260% ==3)
  osend %actor% Вы установили желудь на алтаре, но ничего не произошло.
  osend %actor% Вы забрали драгоценный плод обратно.
  return 0
  halt
end
...
if !%nut260%
  ...
  wait 2s
  say Если ты согласишься помочь мне, я расскажу тебе историю, как в давние времена славный богатырь победил всю болотную нечисть.
...
if !%key260%
  ...
  wait 1
  say %actor.name%, помоги мне найти ключ и я тебе что-то расскажу.
end
~
#26013
...
*добавил наградку вместо пера и слов про мельницу (см ниже)
wait 2
msend       %actor% %self.iname% протянул Вам увесистый мешочек кун.
mechoaround %actor% %self.iname% протянул %actor.dname% увесистый мешочек кун.
eval temp %actor.gold(+5000)%
*wait 1s
*так как продолжения квеста нема, убираю слова про мельницу и перо
*say Где-то на острове в пещере есть выход к черной мельнице.
*say Там ты, став подмастерьем, сможешь обрести навыки мельника, они пригодятся тебе в дальнейшем.
*say Возьми это перо, оно поможет тебе найти путь к мельнице.
*mload obj 26026
*wait 2

  ...
  if !%actor.haveobj(69601)%
    ...
    *Если емкость не пустая
    if %tmp.val1%>0
      ...
      %echoaround% %actor% %actor.name% взял%actor.g% в руки берестяное ведерко и вылил%actor.g% его содержимое под корень вьюна.
      wait 1s
...
if %flag_black_altar% == 1
  ...
  mat 69639 mecho Кто-то появился здесь в клубах сизого дыма.
  mteleport all 69639

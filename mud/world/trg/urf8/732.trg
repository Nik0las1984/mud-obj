...
if %arg.contains(столб)%
  wait 5
  %send% %actor% Столб зашатался в гнезде. Послышался звук двигающегося камня.
  wait 10
  %send% %actor% Внезапно ваша нога подвернулась на ненадежных камнях...
  %send% %actor% И Вы полетели вниз!
  wait 10
  *%teleport% %actor% 73300
  %send% Вы чувствительно приложились головой о камень. Впрочем ничего страшного - это же кость...
else
  %send% %actor% Что Вы собрались толкать??
end
~
$~

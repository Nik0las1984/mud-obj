...
if (!%questor79% || (%questor79.id% != %actor.id%))
  msend %actor% - Спасибо тебе, помог%actor.q% ты мне очень.
...
if (!%killer79% || (%killer79.id% != %questor79.id%))
  ...
  msend %actor% - Ну спасибо тебе, %actor.iname%. Век тебя не забуду.
  ...
  switch %actor.class%
    ...
    case 13
      дум
      г Волхв? Вы сами все умеете делать, нечем мне тебе помочь.
    break
    default
    break
  done

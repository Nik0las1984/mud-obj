...
wait 3ms
wecho _Звук ломающегося камня донесся до вас, сейчас что-то случится
...
if %world.curmobs(47611)% == 0
  wecho &RРунные символы на стене загорелись ярким пламенем&n
  wait 2ms
  wecho Каменная стена отъехала вбок
...
if %num% == 1
  ...
  wechoaround %actor.name% %actor.name% осторожно идет, прощупывая дно болота своей палкой.
  halt
else 
  wsend %actor% _Вы оступились и Вас стало засасывать в болотные топи, но у вас есть еще секунда на рекол.
  wait 3s
  wsend %actor% _Шанса спастить больше нет, как и воздуха, чтобы дышать.
  wechoaround %actor% %actor.name% внезапно нырнул%actor.g% с головой в болото и задохнул%actor.u%.
  set ind 0

...
if %actor% == %yozhmertv%
  ...
  if %actor% == %koldunmertv%
    ...
    wechoaround %actor% И назвал%actor.g% причину всех несчастий - злобного колдуна.
    wait 3s
    ...
    switch %random.2%
      ...
      case 2
        if (%world.curobjs(12717)% < 1)
          ...
          wsend %actor% Медведь протянул %belt.vname% Вам.
          wechoaround %actor% Медведь протянул %belt.vname% %actor.dname%.
    ...
    wecho Медведь утвердительно кивнул.
    wecho - Знаю, знаю, а как же - сорока за вами летала, все мне рассказала.
    wecho - Но вот всех-то ты убил%actor.g%, а так и не выяснил%actor.g%, откуда же
...
if !%self.affect(обездвижен)%
  ...
  while %zigr%
    if %zigr.vnum% != 12731
      msend %zigr% &RЕж СМЕРТЕЛЬНО уколол Вас.&n

...
If (%object.vnum%==721)
  If (%object.val1%>5)
    ...
    If  !(%self.haveobj(60064)%)
      Mload obj 60064
    Else 
      Say Не... минерал у меня уже есть. Второй не нужно пока.
    End
  Else
...
if (%self.haveobj(60064)% && %self.haveobj(60070)%)
  ...
  брос все
  mat 24028 mecho Кто-то появился здесь в клубах сизого дыма.
  mteleport all 24028
end
if (%self.haveobj(60064)% && %self.haveobj(60060)%)
  ...
  mecho Ворожея что-то пошептала над ингредиентами и поднесла их к идолу.
  брос все
  mat 60015 mecho Кто-то появился здесь в клубах сизого дыма.
  mteleport all 60015
end   
if (%self.haveobj(60070)% && %self.haveobj(60060)%)
  ...
  mecho Ворожея что-то пошептала над ингредиентами и поднесла их к идолу.
  брос все
  mat 72035 mecho Кто-то появился здесь в клубах сизого дыма.
  mteleport all 72035
end
~
#49912
...
if %actor.quested(49911)%
  ...
  mecho _- Ну что же... Нашелся ручатель за тебя, а коли так - согласен я принять тебя в гильдию.
  ...
  mechoaround %actor% Мастер Ефраим дал %actor.vname% несколько наставлений по ювелирному искусству.
  mskillturn %actor% ювелир set
  %actor.setquest(49950)%
  %actor.unsetquest(49911)%
  halt
end
mechoaround %actor% Золотых дел мастер внимательно посмотрел на %actor.vname%.
msend %actor% золотых дел мастер внимательно посмотрел на Вас.
say Здрав%actor.g% будь.
say Зачем пожаловал%actor.g%?
вопрос .%actor.name%
~
#49920
